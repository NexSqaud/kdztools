		...
		def loadHeader(self, file):
				...
				for key in self._dz_collapsibles:
						if type(dz_item[key]) is str or type(dz_item[key]) is bytes:
								...
								if dz_item[key] != 0:
										print('[!] Warning: Value supposed to be zero in field "'+key+'" is non-zero ('+hex(dz_item[key])+')', file=sys.stderr)
										# sys.exit(1)
						else:
				...
				print(type(dz_item['pad']))
				if len(str(dz_item['pad'])) != 0:
...
class UNDZFile(dz.DZFile, UNDZUtils):
		...
		def open(self, name):
				...
				self.unknown6 = dz_file['unknown6']
				self.unknown7 = dz_file['unknown7']
				self.unknown8 = dz_file['unknown8']
				self.unknown9 = dz_file['unknown9']
				self.reserved5 = dz_file['reserved5']
...
# FIXME: what if chunks out of order?
		...
		def saveHeader(self, name):
				...
				params.write("unknown6={:s}\n".format(b2a_hex(self.unknown6).decode("utf8")))
				params.write("unknown7={:s}\n".format(b2a_hex(self.unknown7).decode("utf8")))
				params.write("unknown8={:s}\n".format(b2a_hex(self.unknown8).decode("utf8")))
				params.write("unknown9={:s}\n".format(b2a_hex(self.unknown9).decode("utf8")))
				params.write("reserved5={:d}\n".format(self.reserved5))
